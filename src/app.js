...
async function seedInitialData() {
  try {
    ...
    const categories = await Category.bulkCreate([
      { name: 'Technology', icon: 'laptop', color: '#007AFF', isPopular: true },
      { name: 'Design', icon: 'brush', color: '#FF2D55', isPopular: true },
      { name: 'Marketing', icon: 'megaphone', color: '#5856D6', isPopular: true },
      { name: 'Sales', icon: 'trending-up', color: '#FF9500', isPopular: false },
      { name: 'Customer Service', icon: 'headset', color: '#34C759', isPopular: false }
    ], { returning: true });

    // Verify categories were created
    const createdCategories = await Category.findAll();
    console.log('Created categories:', createdCategories.map(cat => ({ id: cat.id, name: cat.name })));
